...
<template>
  <div>
    ...
    <el-card>
      <el-row :gutter="20">
        <el-col :span="4">
          <el-input v-model="queryInfo.carArea" placeholder="请输入停车区域">
          </el-input>
        </el-col>
        <el-col :span="4">
      ...
      <el-table v-loading="loading" :data="cars" border stripe>
        ...
        <el-table-column label="车位区域" prop="stallArea" align="center">
        </el-table-column>
        <el-table-column label="车位类型" prop="stallType" align="center">
        </el-table-column>
        <el-table-column label="车位状态" prop="stallState" align="center">
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="200" fixed="right">
          <template slot-scope="scope">
            <el-button @click="toEdit(scope.row)" type="primary" circle
              ><i class="iconfont icon-edit-button" style="font-size: 22px"></i
            ></el-button>
            <el-button @click="carDelete(scope.row.sid)" type="danger" circle
    ...
    >
      ...
      >
        ...
        <el-form-item label="车位区域" prop="stallArea" class="is-required">
          <el-input v-model="editCar.stallArea" placeholder="请输入车位区域">
          </el-input>
        </el-form-item>
        <el-form-item label="车位类型" prop="stallType" class="is-required">
...
export default {
  ...
  methods: {
    getList() {
      axios.post("/api/stall/pageStall", this.queryInfo).then((res) => {
        this.loading = false;
    ...
    async useStall(sid) {
      ...
      if (re != "confirm") {
        ...
        axios
          .get("/api/stall/orderStall?uid=" + this.uid + "&sid=" + sid)
          .then((res) => {
            if (res.data.data) {

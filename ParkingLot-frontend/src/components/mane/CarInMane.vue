  ...
  <div>
    ...
    <el-card>
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-input v-model="queryInfo.stallArea" placeholder="请输入区域">
          </el-input>
        </el-col>
        <el-col :span="10">
          <el-date-picker
      ...
      <el-table v-loading="loading" :data="stallRes" border stripe>
        ...
        <el-table-column label="收费" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.overTime"
              >{{ scope.row.money.toFixed(2) }}元</span
            >
            <span v-else class="fee-in"
              >{{ feeChange(scope.row).toFixed(2) }}元</span
            >
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            ...
            >
              <el-button
                ...
                disabled
                ><i
                  class="iconfont icon-reduce-button"
                  style="font-size: 22px"
                ></i
              ></el-button>
              <el-button v-else @click="opare(scope.row)" type="warning" circle
                ><i
                  class="iconfont icon-reduce-button"
                  style="font-size: 22px"
                ></i
